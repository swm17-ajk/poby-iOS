...
**목표**: 카메라 안에 들어온 사람이 가이드와 비슷한 **위치/크기(구도)** 로 들어오면 라인이 민트색으로 변경. 포즈(팔다리 각도, 고개 방향 등)는 의도적으로 매칭 대상 아님 — 우리 앱은 "구도 가이드"이지 "포즈 복제"가 아님.
...
- **bounding box로 단순화 — 의도된 설계 선택**:
  - 가이드 silhouette의 외곽선 전체가 아니라 **bbox (감싸는 사각형)** 만 추출
  - 카메라 프레임에서도 사람 마스크의 **bbox** 만 추출 (서브샘플링으로 빠르게)
  - **왜 픽셀 단위 IoU가 아니라 bbox인가**:
    - 우리 가설은 "구도(위치/크기) 가이드"가 사진을 잘 찍게 한다는 것. 포즈 복제가 아님
    - 픽셀 매칭은 가이드 인물의 미세한 포즈까지 따라하라고 요구 → 찍히는 사람에게 부담 → 의도와 어긋남
    - bbox는 정확히 우리가 신경 쓰는 위치/크기를 잡고, 신경 안 쓰는 포즈는 무시
    - 성능 우수 (실시간 fps 부담 ↓, 배터리 친화)
    - 튜닝 직관적 ("사각형 절반 좀 더 겹침" 같은 식으로 임계값 해석 가능)
- **비율 정규화 (4단계와 같은 문제)**:
  - 가이드 bbox: 가이드 원본 사진 비율 (`sourceAspectRatio`) 기준 0-1 좌표
  - 카메라 사람 bbox: 카메라 프레임 비율 기준 0-1 좌표
  - 두 비율 다르면 IoU 계산 전에 같은 좌표계로 변환 — 가이드를 카메라 비율로 letterbox 매핑 (Stage 4 렌더와 동일한 변환)
- 매칭 점수 계산:
  - 변환된 가이드 bbox vs 현재 사람 bbox의 **IoU (Intersection over Union)**
  - 임계값 **0.55** — 실기기 사용 중 튜닝
